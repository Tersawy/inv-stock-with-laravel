				...
				<b-row>
					<b-col cols="9">
						...
						<b-card class="my-30px">
							<products-auto-complete :invoice="invoice" :invoiceFieldName="invoiceFieldName" check-quantity />
							<input-error :vuelidate="this.$v.invoice.products" field="products" :namespace="namespace" />
						</b-card>

						<!-- -------------Products Table------------- -->
						<b-card>
							<invoice-details-table :invoice="invoice" :invoiceFieldName="invoiceFieldName" :namespace="namespace" :net="net" check-quantity />
						</b-card>
...
<script>
	...
	export default {
		...
		data() {
			return { namespace: "Sale", invoiceFieldName: "price" };
		}
	};
</script>
